# Compatibility Report: Controlled Top-N Halo-Count Scaling: Static GCN and EvolveGCN-H at U500

Verdict: **PASS**

Controlled within each model for Top-N. Static-versus-Evolve comparisons are descriptive because temporal input, architecture, depth, batch size, and heads differ.
...
| fixed_scientific_settings.config.num_snapshots | Evolve uses five snapshots; Static consumes one final snapshot. |
| fixed_scientific_settings.config.temporal_pooling | Mean temporal pooling applies only to Evolve. |
| fixed_scientific_settings.dataset_metadata.dataset_type | Evolve datasets are temporal; Static consumes final-snapshot inputs, including the final element of the Top500 temporal container. |
| fixed_scientific_settings.dataset_metadata.num_snapshots | Dataset storage reflects temporal sequences versus final-snapshot graphs. |
