# Compatibility Report: Controlled kNN Connectivity Ablation: Static GCN versus EvolveGCN-H at 500 Universes, Top100 Halos, and Hidden Dimension 64

Verdict: **PASS**

Mostly controlled: matched population, graph protocol, k values, hidden dimension, seeds, and exact split IDs, with intentional model and temporal-input differences.
...
| fixed_scientific_settings.config.hidden_dim | 64 |
...
| field | reason |
|---|---|
| fixed_scientific_settings.config.batch_size | The established model-specific h64 protocols use batch sizes 4 and 8. |
...
| fixed_scientific_settings.config.model | The analysis intentionally compares the EvolveGCN-H and Static GCN architectures. |
| fixed_scientific_settings.config.num_layers | The established h64 protocols use two EvolveGCN-H layers and three Static GCN layers. |
