...
"""
build_temporal_sequences_top2000.py
...
TOP2000_BUILDER_MODULE = "src.data.build_temporal_sequences_top2000"
TOP2000_BUILD_LAUNCHER = "scripts/production/run_u1000_top2000_sparse_build.sh"
TOP2000_RAW_DIR = Path("data/raw/CAMELS_SIMBA_1000U")
TOP2000_OUTPUT = Path(
...
TOP2000_TARGETS = Path("outputs/target_inspection_1000u.csv")


def validate_top2000_production_config(**actual: Any) -> None:
    """Reject every non-canonical invocation of the dedicated Top2000 builder."""
    path_keys = {"raw_dir", "output_path", "targets_csv", "build_launcher_path"}
    normalized = {
        key: (None if value is None else Path(value).as_posix()) if key in path_keys else value
        for key, value in actual.items()
    }
    expected = {
        "raw_dir": TOP2000_RAW_DIR.as_posix(),
        "output_path": TOP2000_OUTPUT.as_posix(),
        "num_universes": 1000,
        "num_snapshots": 5,
        "num_nodes": 2000,
        "normalization": "none",
        ...
        "box_size": 25.0,
        "targets_csv": TOP2000_TARGETS.as_posix(),
        "dummy_target": None,
        "device": "cpu",
        "allow_partial": False,
        "graph_storage": GRAPH_STORAGE_SPARSE,
        "overwrite": False,
        "force_unsafe_dense": False,
        "source_manifest_policy": SOURCE_MANIFEST_POLICY_FULL,
        "builder_entrypoint": TOP2000_BUILDER_MODULE,
        "build_launcher_path": TOP2000_BUILD_LAUNCHER,
    }
    mismatches = [
        f"{key}: expected {expected[key]!r}, received {normalized.get(key)!r}"
        for key in expected
        if normalized.get(key) != expected[key]
    ]
    if mismatches:
        raise ValueError(
            "Dedicated Top2000 builder received a non-canonical configuration:\n- "
            + "\n- ".join(mismatches)
        )
...
def build_temporal_dataset(
    ...
    """
    validate_top2000_production_config(
        raw_dir=raw_dir,
    ...
    actual_builder_module = __spec__.name if __spec__ is not None else "src.data.build_temporal_sequences_top2000"
    ...
    if graph_storage == GRAPH_STORAGE_SPARSE and num_nodes >= 1500:
        if builder_entrypoint is None or launcher_relative is None:
            raise ValueError(
                "Top2000 sparse builds require --builder_entrypoint and --build_launcher_path provenance"
            )
        if launcher_relative != "scripts/production/run_u1000_top2000_sparse_build.sh":
            raise ValueError("Top2000 sparse build launcher provenance is not the production launcher")
