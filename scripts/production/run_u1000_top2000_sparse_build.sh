...
readonly OUTPUT_DIR="data/processed/temporal_1000u_none_top2000_periodic_knn_sparse"
readonly OUTPUT_FILE="${OUTPUT_DIR}/camels_1000u_temporal_logmass_none_top2000_periodic_knn_sparse.pt"
readonly MIN_FREE_KIB=$((6 * 1024 * 1024))
readonly SNAPSHOTS=("0.20000" "0.25000" "0.51209" "0.75065" "1.00000")
readonly BUILDER_MODULE="src.data.build_temporal_sequences_top2000"
readonly BUILD_LAUNCHER="scripts/production/run_u1000_top2000_sparse_build.sh"

usage() {
  cat <<'EOF'
Usage: scripts/production/run_u1000_top2000_sparse_build.sh [--preflight-only|--help]
...
[[ -f reports/experiment_registry/u1000_top1500_raw_halo_count_distribution.csv ]] || fail "authoritative raw halo-count audit missing"
...
if [[ -f "$OUTPUT_FILE" && -f "$metadata" && -f "$marker" ]]; then
  if envs/camels-gnn/bin/python scripts/validation/validate_u1000_top2000_sparse_dataset.py >/dev/null 2>&1; then
    fail "valid completed Top2000 dataset already exists; refusing overwrite"
  fi
fi
...
command=(python -m "$BUILDER_MODULE" --builder_entrypoint "$BUILDER_MODULE" --build_launcher_path "$BUILD_LAUNCHER"
  ...
  --num_universes 1000 --num_snapshots 5 --num_nodes 2000 --normalization none --graph_mode knn
...
log="logs/dataset_builds/u1000_top2000_sparse_build_$(date -u +%Y%m%dT%H%M%SZ).log"
export OMP_NUM_THREADS=4 MKL_NUM_THREADS=4 OPENBLAS_NUM_THREADS=4 NUMEXPR_NUM_THREADS=4 CUDA_VISIBLE_DEVICES=""
{
  echo 'U1000 Top2000 sparse production build'; echo "Git commit: $(git rev-parse HEAD)"; echo "Branch: $branch"
...
envs/camels-gnn/bin/python scripts/validation/validate_u1000_top2000_sparse_dataset.py 2>&1 | tee -a "$log"
