...
def posttest(commit):
 ...
 result=dict(frozen_validation_conclusion=fr['exact_frozen_conclusion'],validation_freeze_commit=commit,validation_freeze_sha256=sha(FREEZE),test_metrics_used_for_model_selection=False,test_seed_metrics=rows,test_summary=summ,paired_test_deltas=delta,validation_directions=vd,test_directions=td,direction_agreement=agree,validation_vs_test=conf,hypothesis_table=hyp,figures=figs,fairness_limitations=fr['fairness_classification']); dump(REG/'u1000_top1500_temporal_five_model_postfreeze_test_evaluation.json',result)
